...
void GameOver::listResult()
{
	...
	for (auto hero : getGameLayer()->_CharacterArray)
	{
		...
		// FIX (bug: musuh kena ultimate/bulu Kabuto berubah status jadi "teman" + FC saat tekan
		// Yes kembali ke menu): sama seperti fix di CharacterBase::dead(), _isControlled dipakai
		// BERSAMA oleh Shintenshin (possession asli, grup memang harus di-restore) DAN stun-lock
		// Kabuto (kb_ult_hit/kb_feather_hit -- cuma freeze, BUKAN possession, grup TIDAK boleh
		// berubah). Kalau match berakhir SAAT korban masih dalam window stun Kabuto,
		// changeGroup() di sini dulu tetap jalan tanpa guard -> korban permanen pindah tim di
		// result screen, dan ketidaksesuaian grup itu yang bikin lookup roster/win-stat di bawah
		// (dan di onLeft() saat "Yes") baca slot yang salah -> force close.
		if (hero->_isControlled)
		{
			hero->_isControlled = false;
			if (hero->getController() && hero->getController()->getName() == HeroEnum::Ino)
				hero->changeGroup();
		}
	...
	if (imgSrc)
	{
		...
		if (Cheats < kMaxCheats)
		{
			...
			if (_isWin)
			{
				...
				{
					// FIX: sama seperti guard di listResult() -- _isControlled dipakai bareng oleh
					// stun-lock Kabuto, jadi changeGroup() cuma boleh jalan kalau memang possession Ino asli.
					if (currPlayer->_isControlled)
					{
						currPlayer->_isControlled = false;
						if (currPlayer->getController() && currPlayer->getController()->getName() == HeroEnum::Ino)
