...
public:
	// Was `private`: Kabuto::reviveCorpseAsPuppet() (Dead Soul Jutsu, see Kabuto.hpp) needs to
	// clear an ARBITRARY revived corpse's (any Hero subclass, not just Kabuto's own instance)
	// pending respawn-countdown skull sprite -- `protected` is not enough here since that only
	// grants access through a pointer of the accessing (Kabuto) type, not a plain Hero* to a
	// sibling subclass instance. Nothing outside Hero.hpp touched this before, so widening to
	// `public` is a safe no-op for every other caller.
	...
	virtual void dealloc()
	{
		...
		if (isClone() || isKugutsu() || isSummon())
		{
			...
			// Notify master BEFORE erase/removeFromParent below: masters that cache a raw
			// pointer to their own clone (e.g. Kabuto::_kbActiveCloneRef) must null it out here,
			// since this clone is about to be erased from _CharacterArray and detached from the
			// scene graph -- std::vector<Hero*> does not retain, so once the scene graph's ref
			// drops the object can be freed and any lingering raw pointer to it dangles.
			if (_master)
				_master->onCloneDealloc(this);
...
public:
	// Was `protected`: Kabuto::reviveCorpseAsPuppet() (Dead Soul Jutsu) needs to unschedule this
	// on an ARBITRARY revived corpse (any Hero subclass), same sibling-access reasoning as
	// rebornSprite/rebornLabel above -- forming `&Hero::countDown` is fine from a derived class,
	// but nothing stops callers elsewhere, so promoted fully public rather than relying on that
	// nuance. No other caller existed before this patch.
