...
class Ino : public Hero
{
	...
	void resumeAction(float dt) override
	{
		...
		{
			// GUARD (sama seperti fix di CharacterBase::dead() / GameOver.cpp): _isControlled
			// dipakai BERSAMA oleh Shintenshin (possession asli Ino) DAN stun-lock Kabuto
			// (kb_ult_hit/kb_feather_hit -- cuma freeze, BUKAN possession). Tanpa guard ini,
			// loop di sini ikut menyapu SIAPA SAJA yang _isControlled==true, termasuk korban
			// stun Kabuto -- changeGroup() di bawah menukar tim korban Kabuto secara PERMANEN
			// (bug laporan #1: "musuh berubah jadi teman"), dan setController(nullptr) di sini
			// jalan lebih dulu daripada guard di listResult()/dead(), jadi guard di sana jadi
			// no-op karena kerusakannya sudah terjadi duluan di sini (root cause force close
			// saat "Yes kembali ke menu", bug laporan #5).
			if (!hero->_isControlled || !hero->getController() || hero->getController()->getName() != HeroEnum::Ino)
				continue;
	...
	void setActionResume() override
	{
		...
		for (auto hero : getGameLayer()->_CharacterArray)
		{
			// GUARD -- lihat penjelasan lengkap di Ino::resumeAction() di atas: jangan sentuh
			// hero yang _isControlled-nya berasal dari stun-lock Kabuto, bukan possession Ino.
			if (!hero->_isControlled || !hero->getController() || hero->getController()->getName() != HeroEnum::Ino)
				continue;
